  ...
  <div class="search">
    ...
    <div v-if="listData.length!=0" class="goodsList">
      <div class="booklist-wrap">
        <goodsList @click="test(1)" v-for="(dataItem, dataIndex) in listData" :key="dataIndex" :goods="dataItem"></goodsList>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async getlistData () {
      //获取商品列表
      const data = await getRequest("/weapp/search/helperaction", {
        keyword: this.words
      })
    ...
    async clearHistory () {
      const data = await postRequest("/weapp/search/clearhistoryAction", {
        openId: this.openid
      });
      console.log(data);
      if (data) {
    ...
    async searchWords (item) {
      ...
      const data = await postRequest("/weapp/search/addhistoryaction", {
        openId: this.openid,
    ...
    async getHotData () {
      const data = await getRequest("/weapp/search/indexaction", {
        openId: this.openid
      });
